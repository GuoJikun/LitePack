...
/// 判断当前进程参数是否包含 CLI 子命令（pack / unpack / list）。
/// GUI 启动前调用，若返回 true 则走 CLI 流程而非启动 Tauri。
pub fn is_cli_subcommand() -> bool {
    std::env::args()
        .nth(1)
        .map(|a| {
...
/// CLI 入口：解析参数 → 执行 → 返回退出码。
/// 供 GUI 二进制在检测到 CLI 子命令后直接调用。
pub fn run_cli() -> i32 {
    let cli = Cli::parse();
    run(cli)
}
